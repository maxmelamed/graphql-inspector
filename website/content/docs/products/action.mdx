...
Minimum severity of inline annotations (`notice` by default).

| Value     | Inline annotations                                  |
| --------- | --------------------------------------------------- |
| `notice`  | All changes (breaking, dangerous, and non-breaking) |
| `warning` | Breaking and dangerous changes                      |
| `failure` | Breaking changes only                               |

To show only breaking changes inline:
...
The level applies after any configured `rules`. It only filters inline annotations: the check
summary and `changes` output still include all changes, and the check conclusion still respects
`fail-on-breaking` and `approve-label`. Setting `annotations: false` disables all inline annotations
regardless of the level. Invalid levels fail the action with a configuration error.
...
Apply [rules](../commands/diff#rules) included with the Inspector as well as your own
[custom rules](../commands/diff#custom-rules)
...
[`considerUsage`](../commands/diff#considerusage) rule
